...
$footer_text = '<center>Nasa IOT register login</center>
                <center> 
                  <a href="register.php">[Register]</a>
                  <a href="stat.php" target="_blank">[Stat]</a>
                </center>';

#GET last input user name
$usr='';
session_start();
if($_GET['UserName']){
  $_SESSION['usr']=$_GET['UserName'];
}else if($_SESSION['usr']){
  $usr=$_SESSION['usr'];
}else{
  echo "No UserName input!";
}
echo 'Input user name:'.$usr;
...
if ($_GET['login'] == login) {
  ...
  $titel = 'Logging in to Nasa IOT register'; 
  $headline = 'Logging in to Nasa IOT register';
  $bodytext = '';
...
if ($_GET['res'] == success) {
  ...
  $bodytext = 'Welcome<center><img src=\'img/loggin.jpg\' height=\'100px\' width=\'auto\'/></center>';
 // $body_onload = 'onLoad="javascript:popUp(' . $loginpath . '?res=popup&uamip=' . $_GET['uamip'] . '&uamport=' . $_GET['uamport'] . '&timeleft='  . $_GET['timeleft'] . ')"';
  ...
  print '<center>
      <form action="http://'.$_man_addr.'/manager.php" method="post" name="red" target="_blank">
      <input type="hidden" name="usr" value="'.$usr.'">
      <input type="submit" name="submit" value="add IOT mac address">
      </form>
      <a href="http://' . $_GET['uamip'] . ':' . $_GET['uamport'] . '/logoff">Logout</a></center>';
...
if ($_GET['res'] == failed) {
  ...
  $result = 2;
  $titel = 'Nasa IOT register Login Failed';
  $headline = 'Nasa IOT register Login Failed';
...
if ($_GET['res'] == logoff) {
  ...
  $result = 3;
  $titel = 'Logged out from Nasa IOT register';
  $headline = 'Logged out from Nasa IOT register';
  $bodytext = '<center>Bye~~</center><center><img src=\'img/logout.jpg\' height=\'100px\' width=\'auto\'/></center><a href="http://' . $_GET['uamip'] . ':' . $_GET['uamport'] . '/prelogin">Login</a>';
...
if ($_GET['res'] == notyet) {
  ...
  $bodytext = "<center><img src='img/headshot.jpg' height='100px' width='auto'/></center>Please log in<br>";
...
function print_body(){
  ...
  print "
    ...
    <body onLoad=\"javascript:doOnLoad($result, '$loginpath?res=popup2&uamip=$uamip&uamport=$uamport&userurl=$userurl&redirurl=$redirurl&timeleft=$timeleft','$userurldecode', '$redirurldecode', '$timeleft')\" onBlur = \"javascript:doOnBlur($result)\" bgColor = '#FBD7D3'>
